...
class ExerciseGrid extends Component {
  ...
  fetchExercisesByCategory = async () => {
    try {
      const response = await axios.get('http://localhost:8000/exercises/categories');
  ...
  handleCategoryClick = async (category) => {
    try {
      const response = await axios.get(`http://localhost:8000/exercises/categories/${category}`);
  ...
  handleStartExercise = async () => {
    ...
    if (cSyntaxExercises && cSyntaxExercises.length > 0) {
      ...
      try {
        const response = await axios.get(`http://localhost:8000/exercise/get/beginners/${firstExerciseId}`);
        this.setState({
          exercise: response.data.exercise,
          loading: false
        });
...
export default withRouter(ExerciseGrid);
